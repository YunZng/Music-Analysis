...
def load_and_preprocess(data, columns):
    try:
        # Load dataset
        songs = pd.DataFrame(data, columns=columns)
...
def predict_recommand(data, columns, song_name):
    songs_data = load_and_preprocess(data, columns)
    recommend(song_name, songs_data)
